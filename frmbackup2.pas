...
begin
  ...
  try
    ...
    begin
      if Pos('S3Lorigem', ListaChaves[i]) = 1 then
        Ini.DeleteKey(Secao, ListaChaves[i]);
    end;

    // Salvar os novos valores
    Ini.WriteString(Secao, 'GDcaminho', FileNameEdit3.FileName);
    Ini.WriteString(Secao, 'S3winrar', FileNameEdit1.FileName);
    Ini.WriteString(Secao, 'S3Ldestino', DirectoryEdit3.Text);

    // Salvar cada linha do Memo1 no INI com os nomes Lorigem1, Lorigem2, etc.
    for i := 0 to Memo1.Lines.Count - 1 do
      Ini.WriteString(Secao, 'S3Lorigem' + IntToStr(i + 1), Memo1.Lines[i]);
...
begin
  ...
  try
    ...
    FileNameEdit1.FileName := Ini.ReadString('ParadaBackup', 'S3winrar', '');    
    DirectoryEdit3.Text := Ini.ReadString('ParadaBackup', 'S3Ldestino', '');
    ...
    i := 1;
    repeat
      Linha := Ini.ReadString('ParadaBackup', 'S3Lorigem' + IntToStr(i), '');
      if Linha <> '' then
...
procedure TForm14.DirectoryEdit1Change(Sender: TObject);
begin
...
procedure TForm14.Memo1MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TForm14.Memo1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  LineIndex: Integer;
begin
...
begin
Form1.BackupLocalS3(Self);
end;

procedure TForm14.FilenameEdit2Change(Sender: TObject);
begin
