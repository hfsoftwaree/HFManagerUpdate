...
begin
  ...
  try
    ...
    begin
      if Pos('S2Lorigem', ListaChaves[i]) = 1 then
        Ini.DeleteKey(Secao, ListaChaves[i]);
    end;

    // Salvar os novos valores
    Ini.WriteString(Secao, 'GDcaminho', FileNameEdit3.FileName);
    Ini.WriteString(Secao, 'S2winrar', FileNameEdit1.FileName);
    Ini.WriteString(Secao, 'S2Ldestino', DirectoryEdit3.Text);

    // Salvar cada linha do Memo1 no INI com os nomes Lorigem1, Lorigem2, etc.
    for i := 0 to Memo1.Lines.Count - 1 do
      Ini.WriteString(Secao, 'S2Lorigem' + IntToStr(i + 1), Memo1.Lines[i]);
...
begin
  ...
  try
    ...
    FileNameEdit1.FileName := Ini.ReadString('ParadaBackup', 'S2winrar', '');    
    DirectoryEdit3.Text := Ini.ReadString('ParadaBackup', 'S2Ldestino', '');
    ...
    i := 1;
    repeat
      Linha := Ini.ReadString('ParadaBackup', 'S2Lorigem' + IntToStr(i), '');
      if Linha <> '' then
...
procedure TForm13.DirectoryEdit1Change(Sender: TObject);
begin
...
procedure TForm13.Memo1MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TForm13.Memo1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  LineIndex: Integer;
begin
...
procedure TForm13.Memo1Exit(Sender: TObject);
var
  i: Integer;
begin
  i := 0;
...
begin
Form1.BackupLocalS2(Self);
end;

procedure TForm13.FilenameEdit2Change(Sender: TObject);
begin
