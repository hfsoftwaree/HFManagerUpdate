...
begin
  ...
  try
    ...
    Ini.WriteString('Servidor 2', 'Start', FileNameEdit1.FileName);
    Ini.WriteString('Servidor 2', 'Shutdown', FileNameEdit2.FileName);
    Ini.WriteString('Servidor 2', 'Profile', DirectoryEdit1.Text );
    Ini.WriteString('Servidor 2', 'Log', DirectoryEdit2.Text );
    Ini.WriteString('Servidor 2', 'mpmission', DirectoryEdit3.Text);    


    Ini.WriteString('Servidor 2', 'exe', FileNameEdit4.Text);
    ...
    Ini.WriteString('UpdateMod', 'destinoserver2', Destino);

    // Define o destino como o diretório do arquivo informado em FileNameEdit4, sem "\" no final
    Destino := ExcludeTrailingPathDelimiter(ExtractFilePath(FileNameEdit4.FileName));
    Ini.WriteString('UpdateDayz', 'destinoserver2', Destino);
...
begin
  ...
  Label1.Caption := 'Parametros ' + Form1.Servidor21.Caption;
  ...
  try
    ...
    FileNameEdit1.FileName := Ini.ReadString('Servidor 2', 'Start', '');
    FileNameEdit2.FileName := Ini.ReadString('Servidor 2', 'Shutdown', '');
    DirectoryEdit1.Text := Ini.ReadString('Servidor 2', 'Profile', '');
    DirectoryEdit2.Text := Ini.ReadString('Servidor 2', 'Log', '');
    DirectoryEdit3.Text := Ini.ReadString('Servidor 2', 'mpmission', '');

    FileNameEdit4.FileName := Ini.ReadString('Servidor 2', 'exe', '');
...
begin
  ...
  if (FileNameEdit4.Text <> '')  then
  begin
  If Application.MessageBox('Confirma criação do arquivo shutdowns2.bat?', 'Confirmação',
  mb_YesNo + mb_ICONQUESTION) = idYes then
  begin

  Ini := TIniFile.Create(ExtractFilePath(Application.ExeName) + 'Config\config.ini');
  try
    CaminhoExe := Ini.ReadString('Servidor 2', 'exe', '');
    ...
    BatPath := PastaServidor + 'shutdowns2.bat';
