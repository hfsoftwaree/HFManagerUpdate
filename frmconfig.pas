...
begin
  ...
  try
    ...
    Ini.WriteString('Servidor 1', 'Start', FileNameEdit1.FileName);
    Ini.WriteString('Servidor 1', 'Shutdown', FileNameEdit2.FileName);
    Ini.WriteString('Servidor 1', 'Profile', DirectoryEdit1.Text);
    Ini.WriteString('Servidor 1', 'Log', DirectoryEdit2.Text);
    Ini.WriteString('Servidor 1', 'mpmission', DirectoryEdit3.Text);

    Ini.WriteString('Servidor 1', 'exe', FileNameEdit4.FileName);    
    ...
    Ini.WriteString('UpdateMod', 'destinoserver1', Destino);

    // Define o destino como o diretório do arquivo informado em FileNameEdit4, sem "\" no final
    Destino := ExcludeTrailingPathDelimiter(ExtractFilePath(FileNameEdit4.FileName));
    Ini.WriteString('UpdateDayz', 'destinoserver1', Destino);
...
begin
  ...
  Label1.Caption := 'Parametros ' + Form1.Servidor11.Caption;
  ...
  try
    ...
    FileNameEdit1.FileName := Ini.ReadString('Servidor 1', 'Start', '');
    FileNameEdit2.FileName := Ini.ReadString('Servidor 1', 'Shutdown', '');
    DirectoryEdit1.Text := Ini.ReadString('Servidor 1', 'Profile', '');
    DirectoryEdit2.Text := Ini.ReadString('Servidor 1', 'Log', '');
    DirectoryEdit3.Text := Ini.ReadString('Servidor 1', 'mpmission', '');

    FileNameEdit4.FileName := Ini.ReadString('Servidor 1', 'exe', '');
  finally
    // Libera a memória
    Ini.Free;
  end;
...
begin
  ...
  if (FileNameEdit4.Text <> '')  then
  begin
  If Application.MessageBox('Confirma criação do arquivo shutdowns1.bat?', 'Confirmação',
  mb_YesNo + mb_ICONQUESTION) = idYes then
  begin

  Ini := TIniFile.Create(ExtractFilePath(Application.ExeName) + 'Config\config.ini');
  try
    CaminhoExe := Ini.ReadString('Servidor 1', 'exe', '');
    ...
    BatPath := PastaServidor + 'shutdowns1.bat';
